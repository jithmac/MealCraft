...
:- discontiguous food/13.
:- discontiguous avoid_tag/2.
:- discontiguous prefer_tag/2.
:- discontiguous diet_required_tag/2.
:- discontiguous diet_forbidden_tag/2.
:- discontiguous plan_template/5.
...
valid_meal_plan_calories(TargetCalories, TotalCalories) :-
    TargetCalories < 2200,
    Min is TargetCalories - 300,
    Max is TargetCalories + 450,
    TotalCalories >= Min,
    TotalCalories =< Max.
...
plan_template(standard_2500_omnivore, 2200, 2800, omnivore_group,
    plan(
...
plan_template(standard_2500_vegetarian, 2200, 2800, vegetarian_group,
    plan(
        [item(oats, 2), item(banana, 1), item(milk_low_fat, 1), item(egg_boiled, 2)],
...
plan_template(standard_2500_vegan, 2200, 2800, vegan_group,
    plan(
...
print_item(item(Food, Quantity)) :-
    ...
    write(' serving(s) = '), write(ItemCalories), write(' kcal'), nl.
